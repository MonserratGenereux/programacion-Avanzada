...
static char * testInitialDictionary() {
    unsigned int size=4;
    int errorCode;
    Dict *myDictionary = initDictionary(size,&errorCode);
  
  muAssert("=4", myDictionary->size==4);
...
static char * testUpsertDictionary() {
  ...
  muAssert("myDictionary errorCode must be 0", errorCode==0);
  muAssert("blab", strcmp(myDictionary->element[0].key,"uno")== 0);
  muAssert("blab", *((int *)myDictionary->element[0].key,"uno")== value);
  return 0;
}

static char * allTests() {
  muRunTest(testInitialDictionary);
